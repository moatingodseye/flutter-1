...
class ApiClient {
  ...
  Future<Map<String,dynamic>> login(String username, String password) async {
    final r = await http.post(Uri.parse('\$baseUrl/api/login'), headers: {'content-type':'application/json'}, body: jsonEncode({'username':username,'password':password}));
  ...
  Future<List<dynamic>> getProjects() async {
    final r = await http.get(Uri.parse('\$baseUrl/api/projects'), headers: _headers);
    return jsonDecode(r.body) as List<dynamic>;
  }

  Future<void> createProject(Map<String,dynamic> project) async {
    final r = await http.post(Uri.parse('\$baseUrl/api/projects'), headers: _headers, body: jsonEncode(project));
  ...
  Future<void> updateProject(int id, Map<String,dynamic> project) async {
    final r = await http.put(Uri.parse('\$baseUrl/api/projects/\$id'), headers: _headers, body: jsonEncode(project));
    if (r.statusCode != 200) throw Exception('Update project failed');
  }

  Future<void> deleteProject(int id) async {
    final r = await http.delete(Uri.parse('\$baseUrl/api/projects/\$id'), headers: _headers);
    if (r.statusCode != 200) throw Exception('Delete project failed');
  }

  Future<void> createUser(Map<String,dynamic> user) async {
    final r = await http.post(Uri.parse('\$baseUrl/api/users'), headers: _headers, body: jsonEncode(user));
    if (r.statusCode != 200) throw Exception('Create user failed');
  }

  Future<void> updateUser(int id, Map<String,dynamic> user) async {
    final r = await http.put(Uri.parse('\$baseUrl/api/users/\$id'), headers: _headers, body: jsonEncode(user));
    if (r.statusCode != 200) throw Exception('Update user failed');
  }

  Future<void> deleteUser(int id) async {
    final r = await http.delete(Uri.parse('\$baseUrl/api/users/\$id'), headers: _headers);
    if (r.statusCode != 200) throw Exception('Delete user failed');
  }

  Future<void> addTimeEntry(Map<String,dynamic> entry) async {
    final r = await http.post(Uri.parse('\$baseUrl/api/time_entries'), headers: _headers, body: jsonEncode(entry));
  ...
  Future<List<dynamic>> getTimeEntries({int? userId, String? start, String? end}) async {
    ...
    final url = '\$baseUrl/api/time_entries' + (params.isNotEmpty ? '?'+params.join('&') : '');
  ...
  Future<void> updateTimeEntry(int id, Map<String,dynamic> entry) async {
    final r = await http.put(Uri.parse('\$baseUrl/api/time_entries/\$id'), headers: _headers, body: jsonEncode(entry));
    if (r.statusCode != 200) throw Exception('Update time entry failed');
  }

  Future<void> deleteTimeEntry(int id) async {
    final r = await http.delete(Uri.parse('\$baseUrl/api/time_entries/\$id'), headers: _headers);
